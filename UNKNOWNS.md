...
| Set bonus types with no quality table | Offensive Penetration has no Craftable Sets row; blue is read (1401 for a 1487 gold bonus, the DK's Spellshredder Hat tooltip, 2026-09-19) and matches the resistance table ratio, so white, green and purple use the resistance ratios (`sets.bonusByQuality.offensivePenetration`, unverified there). Critical Resistance (27 bonuses) has no row and no reading: it keeps the gold value with a note. Searched: `grep -i penetration data/reference/tables_index.csv` (0 hits), `grep -i 'penetration\|critical resistance' data/reference/pages_index.csv` (prose only), `grep -i 'set bonus' data/patch-notes/text` (no quality values). |
