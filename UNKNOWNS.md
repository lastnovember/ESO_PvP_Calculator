...
| Armor rating below gold (`items.qualityFactor`) | blue 0.9427 (read: a blue light head is 1151 against 1221 gold, DK tooltip 2026-09-19); purple 0.96, green 0.88, white 0.84 still community steps | The blue reading contradicts the 4% community steps, so purple, green and white are open; one slot and weight read, whether every slot scales the same is not known. Weapons no longer use this (Nirnhoned tables). | `grep -i armor data/reference/tables_index.csv` (uesp_Online_Armor_t01 is the trait table); pages/Armor.md (no ratings); pages/depth/Rubedite_Ore.md, Rubedo_Leather.md, Ancestor_Silk.md tables (ingot counts, not ratings); data/patch-notes/text "armor rating" (none by quality). One reading with a single purple armor piece settles it by differencing. |
...
| Sprint Cost stacking (`STRATEGIES.sprintCostPercents`) | multiplicative per source | Two rules fit all eight sprint readings (001 to 011): the percents multiplying per source with normal rounding, or adding with the result rounded up. A reading separates them where they differ by one: a character with seven light pieces and Sprinter reads 363 under the first rule and 364 under the second, or three light and four medium pieces 402 against 401. | pages/Stealth.md and pages/Movement_Speed.md (passive texts only, no stacking rule); skills.csv Grace and Medium Armor Bonuses texts; data/patch-notes/text "sprint" with "cost" (values, no stacking rule); tables_index.csv "Sprint" (none). |
...
| Set bonus types with no quality table | Offensive Penetration has no Craftable Sets row; blue is read three times for the same 34-1487 line: 1401 on the Spellshredder Hat (armor) and the Spellshredder Ring (jewelry), 1400 on the Coup De Grâce Restoration Staff (weapon), all DK inventory tooltips of 2026-09-19; the resistance table ratio gives 1400.53, which rounds to the first two and truncates to the staff. The odd one out is either the weapon (a per item kind curve) or the Coup De Grâce set (a per set curve); a blue Spellshredder weapon or a blue Coup De Grâce armor or jewelry piece separates the two. Purple is read once (1435 on the Coup De Grâce Amulet, 2026-09-19) and is the resistance ratio exactly (1487 x 2871 / 2975 = 1435.0). The engine keeps the resistance ratio (1401 at blue) and this row stays open for the blue rounding; white and green use the resistance ratios (`sets.bonusByQuality.offensivePenetration`, unverified there). A blue Spellshredder weapon, or a blue Coup De Grâce armor or jewelry piece, separates a per item kind curve from a per set one. Critical Resistance has no row either; blue is read once (399 for a 9-424 line, the DK's blue Greatsword of Transmutation, 2026-09-19), which the resistance ratio reproduces (399.3), so it scales the same way with blue verified and white, green and purple unverified (`sets.bonusByQuality.critResistance`). Searched: `grep -i penetration data/reference/tables_index.csv` (0 hits), `grep -i 'penetration\|critical resistance' data/reference/pages_index.csv` (prose only), `grep -i 'set bonus' data/patch-notes/text` (no quality values). |
...
| Templar ring 1 glyph | Reduce Skill Cost (Reduce Prismatic Cost 133), Protective 1190, Monomyth Reforged: the user's tooltip photo, 2026-09-19 (photos/tooltip-templar-ring1-monomyth-reforged.jpg). The export had said Prismatic Recovery; fixtures 005 and 006 carry the correction and rise to 46 and 48 of 51. |
...
| Roksa the Warped 1 item | 70 Health, Magicka and Stamina Recovery, counted | 005 and 006 (ring 1 carries Reduce Skill Cost, no recovery) |
