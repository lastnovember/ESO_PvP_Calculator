...
| Set bonus types with no quality table | Offensive Penetration has no Craftable Sets row; blue is read twice for the same 34-1487 line and differs by one: 1401 on the Spellshredder Hat (armor) and 1400 on the Coup De Grâce Restoration Staff (weapon), both DK inventory tooltips of 2026-09-19; the resistance table ratio gives 1400.53, which rounds to the hat and truncates to the staff, so either the rounding or the per item curve differs between armor and weapons. Purple is read once (1435 on the Coup De Grâce Amulet, 2026-09-19) and is the resistance ratio exactly (1487 x 2871 / 2975 = 1435.0). The engine keeps the resistance ratio (1401 at blue) and this row stays open for the blue rounding; white and green use the resistance ratios (`sets.bonusByQuality.offensivePenetration`, unverified there). A second blue armor piece and a second blue weapon with the line separate rounding from a per slot curve. Critical Resistance has no row either; blue is read once (399 for a 9-424 line, the DK's blue Greatsword of Transmutation, 2026-09-19), which the resistance ratio reproduces (399.3), so it scales the same way with blue verified and white, green and purple unverified (`sets.bonusByQuality.critResistance`). Searched: `grep -i penetration data/reference/tables_index.csv` (0 hits), `grep -i 'penetration\|critical resistance' data/reference/pages_index.csv` (prose only), `grep -i 'set bonus' data/patch-notes/text` (no quality values). |
