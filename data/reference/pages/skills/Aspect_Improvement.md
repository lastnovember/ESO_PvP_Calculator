# Online:Aspect Improvement

Source: https://en.uesp.net/wiki/Online:Aspect_Improvement
License: CC BY-SA, UESP

Online Skill Summary
id=46763
id1=46758
id2=46759
id3=46760
id4=46763
line=Enchanting
type=Passive
icon=Enchanting-Aspect Improvement
desc=Allows the use of Common (white) and Standard (green) Aspect Runestones.
desc2=Allows the use of Superior (blue) Aspect Runestones.
desc3=Allows the use of Artifact (purple) Aspect Runestones.
desc4=Allows the use of Legendary (gold) Aspect Runestones.
onefree=1
linerank1=1
linerank2=6
linerank3=16
linerank4=31

Aspect Improvement determines the quality of Glyphs you can make. These can be used to enchant your weapons, armor, and jewelry. You start with one free point already in this skill, allowing the crafting of Base and Standard quality Glyphs.

The Aspect Runes used to craft Glyphs are as follows:
aspect

## Patch Notes
Potency Improvement]] to more closely match their associated runestone shapes.
