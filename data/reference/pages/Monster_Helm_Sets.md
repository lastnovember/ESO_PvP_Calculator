# Online:Monster Helm Sets

Source: https://en.uesp.net/wiki/Online:Monster_Helm_Sets
Fetched: 2026-09-12
License: CC BY-SA, UESP

Monster Helm Sets (commonly called "monster heads") are a subset of dungeon sets of armor. As the name suggests, the design of these sets are based on various bosses found in Group Dungeons. They all consist of one head piece and one shoulder piece, and come in every weight and trait (other than Nirnhoned). When both pieces are worn they grant a powerful and unique ability or buff that cannot be obtained elsewhere.

The head pieces are guaranteed drops from the associated last boss of a group dungeon on Veteran difficulty, while shoulder pieces can be bought from Pledge Masters for Undaunted Keys. The head pieces cannot be obtained from Normal mode dungeons. Items from these sets can also sometimes be purchased from The Golden in Cyrodiil for Gold or Alliance Points.

The style of Monster Helm Sets can also be applied to your character using the Outfit System, provided the appropriate Undaunted Style page has been learned.

## Dungeon Sets
Zaan

## Imperial City Sets
Imperial City
Six monster helm sets can be found in the Imperial City. The head pieces are guaranteed drops from the associated boss and have a chance of being found in their respective district's Trophy Vaults, while the shoulder pieces can be bought with Tel Var Stones from Tel Var Grand Armorers: Birstagg]].
Zoal the Ever-Wakeful (set)

## Night Market Sets
Three monster helm sets can be found in the Night Market and obtaining them will depend on what faction you align with for that season. The head pieces can be received in reward coffers for participating in the inter-faction contest, and shoulder pieces can be obtained by purchasing the associated coffer from a Night Market Merchant.
Thousand Eyes (set)

## Tables extracted from this page

- tables/uesp_Online_Monster_Helm_Sets_t00.csv (58 rows): Set Name | Bonuses | Head Piece Dropped by | Shoulder Vendor | Helm | Shoulder
- tables/uesp_Online_Monster_Helm_Sets_t01.csv (6 rows): Set Name | Bonuses | Head Piece Dropped by | Shoulder Vendor | Helm | Shoulder
- tables/uesp_Online_Monster_Helm_Sets_t02.csv (3 rows): Set Name | Bonuses | Head Piece Dropped by | Shoulder Vendor | Helm | Shoulder
