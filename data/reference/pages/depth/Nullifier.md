# Online:Nullifier

Source: https://en.uesp.net/wiki/Online:Nullifier
License: CC BY-SA, UESP

Nullifiers are a class of hostile humanoid NPCs that attack using magic. They usually wield a shock staff or a two-handed sword. They are notable in that they can use a negate to prevent their target from using magic.

## Types of Nullifiers
Location | Name | Race | Gender | Health

Passage of Dad'na Ghaten | Dro-m'Athra Anguish-Eater | Dro-m'Athra | Varies | ba

Doomstone Caverns | Skeletal Nullifier | Khajiit Skeleton | No Gender | a

Hew's Bane | Iron Wheel Nullifier | Varies | a

Hakoshae Tombs | Ancestral Nullifier | Akaviri | Varies | a

## Skills and Abilities

### Two-Handed Sword
;Quick Strike:A basic melee attack dealing minor physical damage.
;Uppercut:An Uppercut-like attack dealing moderate physical damage, knocking back and stunning. This can be blocked to knock the nullifier off balance instead.
;Ice Cage:The nullifier summons a ring of frost around itself dealing continuous direct moderate frost damage and heavily snaring all targets in the red ring. This cannot be blocked.
;Negate Magic:The nullifier creates a globe of magic suppression for 10 seconds, removing and preventing all player area of effect abilities from occurring in the area. Players within the globe will be silenced and pets/companions are stunned. Silence prevents players from using any ability costing Health, Magicka, Ultimate or have no cost at all as long as they are in the affected area.

### Staff
;Shock:A basic ranged attack dealing minor shock damage.
;Impending Storm:The nullifier charges up and releases lightning from itself as indicated by red circles dealing moderate shock damage to all targets hit. This can be interrupted to set the nullifier off balance. This cannot be blocked.
;Negate Magic:The nullifier creates a globe of magic suppression for 10 seconds, removing and preventing all player area of effect abilities from occurring in the area. Players within the globe will be silenced and pets/companions are stunned. Silence prevents players from using any ability costing Health, Magicka, Ultimate or have no cost at all as long as they are in the affected area.

## Tables extracted from this page

- tables/depth_Nullifier_t00.csv (4 rows): Location | Name | Race | Gender | Health
