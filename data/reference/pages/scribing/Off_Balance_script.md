# Online:Off Balance (script)

Source: https://en.uesp.net/wiki/Online:Off_Balance_(script)
License: CC BY-SA, UESP

Off Balance is an Affix Script used in Scribing. It is earned as a reward for The Wing of the Indrik quest.

colspan=2|Grimoire | Effects

Elemental Explosion | Elemental Explosion
Sets enemies Off Balance for 7 seconds.

Shield Throw | Shield Throw
Sets the enemy Off Balance for 7 seconds.

Traveling Knife | Traveling Knife
Sets enemies Off Balance for 7 seconds.

rowspan=2|Vault | rowspan=2|Vault
Sets enemies Off Balance for 7 seconds.

Incompatible with Healing Vault

Trample | Trample
Sets enemies Off Balance for 7 seconds.

## Notes
- The Daedric text on the script reads "Off Balance".

## Tables extracted from this page

- tables/scribing_Off_Balance_script_t00.csv (6 rows): Elemental Explosion | Elemental Explosion | Sets enemies Off Balance for 7 seconds.
